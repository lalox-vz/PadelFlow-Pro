-- 1. Ensure 'price' column exists in bookings for historical accuracy
DO $$
BEGIN
...
-- 2. Create the Dashboard Intelligence RPC
...
BEGIN
    ...
    -- 2. Breakdown
    SELECT 
        COALESCE(SUM(CASE WHEN recurring_plan_id IS NULL THEN price ELSE 0 END), 0),
        COALESCE(SUM(CASE WHEN recurring_plan_id IS NOT NULL THEN price ELSE 0 END), 0)
    ...
    -- 3. Active Members (Unique users in bookings + Recurring Plans active)
    -- Simplified: Count distinct users who booked in this period
    ...
    -- 5. Heatmap Data
    -- Frequency of bookings by Day of Week (0-6) and Hour (0-23)
