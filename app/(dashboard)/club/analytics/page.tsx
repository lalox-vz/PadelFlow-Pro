...
export default function AnalyticsPage() {
    ...
    }, [loading, isOwner, router, profile])

    useEffect(() => {
        if (orgId && isOwner) fetchData()
    }, [orgId, isOwner])
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Map SQL courts to expected structure if needed, or just use as is
            const courtList = courtData || []

            // 2. Get Bookings (Last 30 days)
            const startDate = subDays(new Date(), 30).toISOString()

            // Use broader query to be safe about entity_id column existence
            // We join on courts table if needed, but for now assuming entity_id works based on previous code.
            // If entity_id fails, this will throw.
    ...
    // Simple logic: Hours with < 20% of peak traffic
        ...
        <div className="space-y-8 pb-10">
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-zinc-400">Rendimiento y ocupación de los últimos 30 días.</p>
                </div>
